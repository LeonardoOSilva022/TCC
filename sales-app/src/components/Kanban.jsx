...
export default function Kanban({
  ...
  const isEditingMode =
    !isManagement && selectedOrder?.status === "Aguardando Correção";
    ...
    <div className="h-full flex flex-col space-y-4 font-sans">
      <div className="lg:hidden flex gap-2 overflow-x-auto pb-2 no-scrollbar px-1">
        {COLUMNS.map((col) => (
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-full text-[10px] font-bold border transition-all ${mobileTab === col.id ? "bg-blue-600 border-blue-500 text-white" : "bg-neutral-900 border-neutral-800 text-neutral-500"}`}
          >
      ...
      <div className="flex-1 flex lg:flex-row flex-col gap-6 overflow-x-auto lg:overflow-visible">
        {COLUMNS.map((column) => (
          <div
            key={column.id}
            className={`${mobileTab === column.id ? "flex" : "hidden lg:flex"} min-w-[310px] lg:w-[320px] bg-neutral-900/40 rounded-3xl border border-neutral-800 flex flex-col max-h-full`}
          >
            <div
              className={`hidden lg:flex p-4 border-b border-neutral-800 items-center justify-between bg-neutral-900/50 rounded-t-3xl ${column.id === "Aguardando Correção" ? "border-b-red-500/20" : ""}`}
            >
              <h3
                className={`text-xs font-bold uppercase tracking-widest flex items-center gap-2 ${column.id === "Aguardando Correção" ? "text-red-500" : "text-neutral-400"}`}
              >
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-mono text-neutral-600 bg-black px-1.5 py-0.5 rounded">
                            #{order.id}
                          </span>
                          {isCorrection && (
                        ...
                        <span className="text-[9px] font-bold text-blue-500/70 uppercase truncate max-w-[100px]">
                          {seller?.name || "Sistema"}
                        </span>
                      </div>

                      <h4 className="font-bold text-white text-sm truncate mb-3">
                        {order.client}
                      </h4>

                      <div className="flex justify-between items-center pt-3 border-t border-neutral-800/50">
                        <span
                          className={`font-black text-sm ${isCorrection ? "text-red-400" : "text-blue-400"}`}
                        >
          ...
          <div className="bg-neutral-900 w-full lg:max-w-5xl h-[92vh] lg:h-auto lg:rounded-3xl border-t lg:border border-neutral-800 flex flex-col overflow-hidden animate-slide-up">
            <header className="p-5 border-b border-neutral-800 flex justify-between items-center shrink-0 bg-black/20">
              <div>
                <h2 className="text-base font-bold text-white flex items-center gap-2">
                  ...
                  {isEditingMode && (
                    <span className="text-[9px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded font-black uppercase tracking-widest border border-yellow-500/20">
                      Modo de Correção
                    </span>
                  )}
                </h2>
              ...
              <button
                ...
                className="p-2 bg-neutral-800 text-neutral-400 hover:text-white rounded-full transition-all"
              >
                <X size={20} />
              </button>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-5 lg:p-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-6">
                <h3 className="text-[10px] font-black text-neutral-600 uppercase tracking-[0.2em] flex items-center gap-2">
                  <List size={14} /> Itens da Operação
                </h3>
                  ...
                  {isEditingMode && (
                    <div className="p-3 bg-neutral-900/50">
                      <select
                        ...
                        defaultValue=""
                        className="w-full bg-black border border-neutral-800 p-2.5 rounded-lg text-xs text-neutral-400 outline-none focus:border-blue-500 cursor-pointer"
                      >
                ...
                <div className="pt-4 border-t border-neutral-800">
                  ...
                  {isManagement &&
                    ["Aguardando Aprovação", "Aprovado"].includes(
                      selectedOrder.status,
                    ) && (
                      <div className="p-4 bg-blue-600/5 border border-blue-500/20 rounded-2xl mb-6">
                        <p className="text-[9px] font-black text-blue-500 uppercase mb-2 flex items-center gap-1">
                          <AlertTriangle size={12} /> Aplicar Desconto de
                          Negociação
                        </p>
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="bg-blue-600 hover:bg-blue-500 px-4 py-2 rounded-lg text-[10px] text-white font-bold transition-all"
                          >
                            APLICAR
                          </button>
                        </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {isManagement &&
                    ["Aguardando Aprovação", "Aguardando Correção"].includes(
                      selectedOrder.status,
                    ) ? (
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          onClick={() => {
                            updateStatus(selectedOrder.id, "Aprovado");
                            setSelectedOrderId(null);
                          }}
                          className="bg-blue-600 py-3 rounded-xl font-bold text-[10px] uppercase flex flex-col items-center gap-1 shadow-lg shadow-blue-900/20"
                        >
                        ...
                        <button
                          ...
                          className="bg-yellow-500/10 text-yellow-500 py-3 rounded-xl border border-yellow-500/20 font-bold text-[10px] uppercase flex flex-col items-center gap-1"
                        >
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm("Excluir este pedido?")) {
                              updateStatus(selectedOrder.id, "Cancelado");
                              setSelectedOrderId(null);
                            }
                          }}
                          className="bg-red-500/10 text-red-500 py-3 rounded-xl border border-red-500/20 font-bold text-[10px] uppercase flex flex-col items-center gap-1"
                        >
                      ...
                      // TRAVA DE SEGURANÇA: Só exibe o botão de pagamento para Gestores
                      isManagement ? (
                        <button
                          onClick={() => {
                            updateStatus(selectedOrder.id, "Pago");
                            setSelectedOrderId(null);
                          }}
                          className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 transition-all shadow-lg hover:bg-emerald-500"
                        >
                          <DollarSign size={16} /> Confirmar Pagamento
                        </button>
                      ) : (
                        // Se for vendedor, ele apenas vê o aviso de que aguarda o financeiro
                        <div className="p-4 bg-neutral-800/20 border border-neutral-800 rounded-2xl flex items-center gap-3 italic text-[10px] text-neutral-500">
                          <Clock size={12} /> Aguardando confirmação do setor
                          financeiro...
                        </div>
                      )
                      ...
                      <button
                        onClick={() => {
                          updateStatus(selectedOrder.id, "Em Separação");
                          setSelectedOrderId(null);
                        }}
                        className="w-full bg-purple-600 text-white py-4 rounded-xl font-bold text-xs uppercase flex items-center justify-center gap-2 transition-all"
                      >
                        <Package size={16} /> Enviar para Logística
                      </button>
                    ) : (
                      <div className="p-4 bg-neutral-800/20 border border-neutral-800 rounded-2xl flex items-center gap-3 italic text-[10px] text-neutral-500">
                        <Lock size={12} /> Status atual do pedido:{" "}
                        {selectedOrder.status}
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-6">
                <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl">
                  <h3 className="text-[10px] font-black text-neutral-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                    <History size={14} /> Rastreio do Cliente
                  </h3>
                  <div className="relative border-l border-neutral-800 ml-2 space-y-5 pb-2">
                    {(selectedOrder.history || []).map((h, i) => (
                      <div key={i} className="pl-5 relative animate-fade-in">
                        <div
                          className={`absolute -left-[5px] top-1 w-2.5 h-2.5 rounded-full ${i === selectedOrder.history.length - 1 ? "bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]" : "bg-neutral-600"}`}
                        ></div>
                        <p
                          className={`text-[11px] font-black uppercase ${i === selectedOrder.history.length - 1 ? "text-white" : "text-neutral-400"}`}
                        >
                          {h.status}
                        </p>
                ...
                {["Aguardando Aprovação", "Aprovado"].includes(
                  ...
                  <div className="bg-white p-4 rounded-3xl flex flex-col items-center gap-4 shadow-xl border border-neutral-200">
                    ...
                    <button
                      ...
                      className="w-full bg-emerald-600/10 text-emerald-600 py-3 rounded-xl text-[10px] font-black border border-emerald-600/20 uppercase tracking-widest hover:bg-emerald-600/20 transition-all"
                    >
                      Copiar Chave PIX
                    </button>
                  </div>
                ...
                <button
                  onClick={() => generateBoletoPDF(selectedOrder)}
                  className="w-full bg-neutral-100 text-black py-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg hover:bg-white transition-all"
                >
                  <Printer size={16} /> Gerar Boleto Bancário
                </button>
              </div>
            </div>
          </div>
